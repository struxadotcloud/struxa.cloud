...
const downloadClass = 'inline-flex min-h-11 items-center text-xs text-neutral-300 underline decoration-neutral-600 underline-offset-4 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-blue-400';
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      ...
      <header className="px-6 pb-10 pt-16">
        <p className="text-xs uppercase tracking-wide text-blue-400">Brand resources</p>
      ...
      <section aria-labelledby="logos-heading" className="px-6 py-10">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {variants.map((variant) => (
            <figure key={variant.key}>
              <div className="flex aspect-[2/1] items-center justify-center gap-6 rounded-2xl border border-white/10 px-6 sm:px-10" style={{ backgroundColor: variant.background }}>
      ...
      <section aria-labelledby="colors-heading" className="px-6 py-10">
        ...
        <div className="mt-8 grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-800 sm:grid-cols-2 lg:grid-cols-4">
          {colors.map((color) => (
            <div key={color.hex} className="flex min-h-48 flex-col justify-between p-6" style={{ backgroundColor: color.hex, color: color.text }}>
              ...
              <div className="mt-10 text-sm"><p>{color.hex}</p><p className="mt-1 text-xs">RGB {color.rgb}</p></div>
            </div>
          ))}
        </div>
      </section>

      <section aria-labelledby="graphics-heading" className="px-6 py-10">
